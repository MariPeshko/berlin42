...
/*   Updated: 2024/07/18 19:28:23 by mpeshko          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	multi_pipe(int argc, char **argv, char **env)
{
	...
	int	read_from;
	int	write_to;
	// transform it to linked list, to assign NULL in the end?
	int	ids[100] = {-3, -2, -4, -1};
	int	*ptr;
	int	children;

	ptr = ids;
	// it's not correct. we need to count only valid commands
	// or avoid this variable if I store all in linked list.
	children = argc - 3 - 1;
