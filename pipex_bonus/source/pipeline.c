...
/*   Updated: 2024/06/18 20:19:22 by mpeshko          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	pipeline_right(char **argv, char **env, int *pipe_ends)
{
	...
	outfile_fd = open(argv[7], O_WRONLY | O_TRUNC | O_CREAT, 0644);
	if (!*argv)
		perror_and_exit("No last command", pipe_ends);
	if (close(pipe_ends[1]) == -1)
		perror_and_exit("close", pipe_ends);
	if (outfile_fd == -1)
	{
...
void	parent_process(int child_id_1, int child_id_2, int *pipe_ends, int n)
{
	int	child_status;
	...
	fprintf(stderr, "%d %i child_id_2: %i \n", child_id_1, n, child_id_2);
