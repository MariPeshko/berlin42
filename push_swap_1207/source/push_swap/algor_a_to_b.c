...
// Search for number of operations for pushing 'nbr' from stack a
// to stack b. Test case with both rotations (to the right) ft_rrr
// and then either ft_rra or ft_rrb. It suits when the number and
// its place in the end of both stacks.
// I added 12.07
...
// to find a smallest number of operations to push a number.
// Variable 'c' stands for the "cheapest cost".
int alg_cheapest_ab(t_stack *a, t_stack *b)
{
	...
	temp = a;
	c = case_two(a, b, a->nbr);
	while (temp)
	{
		if (c > case_one(a, b, temp->nbr))
			c = case_one(a, b, temp->nbr);
		if (c > case_two(a, b, temp->nbr))
			c = case_two(a, b, temp->nbr);
		temp = temp->next;
	}
	return (c);
}
